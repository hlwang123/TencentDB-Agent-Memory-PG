...
load_env
require_vars MEMORY_CORE_IMAGE MEMORY_CORE_PORT MEMORY_CORE_VOLUME PG_CONNECTION_STRING
...
memory:
  ...
  postgres:
    # PG 连接串从 .env 的 PG_CONNECTION_STRING 读取，例如：
    # postgres://<user>:<password>@<pg-host>:5432/tdai_memory
    connectionString: "${PG_CONNECTION_STRING}"
  embedding:
    provider: ${EMBEDDING_PROVIDER:-none}
    baseUrl: "${EMBEDDING_BASE_URL:-}"
    apiKey: "${EMBEDDING_API_KEY:-}"
    model: "${EMBEDDING_MODEL:-}"
    dimensions: ${EMBEDDING_DIMENSIONS:-1024}
    sendDimensions: ${EMBEDDING_SEND_DIMENSIONS:-true}
...
# ── Hotfix: sendDimensions missing in store-pool.ts ──
info "  Applying sendDimensions hotfix..."
if ! docker exec "$CONTAINER" grep -qF "[hotfix] sendDimensions" /app/src/core/store/store-pool.ts 2>/dev/null; then
  docker exec "$CONTAINER" sed -i '/maxInputChars: embCfg.maxInputChars,/a\        sendDimensions: embCfg.sendDimensions,' /app/src/core/store/store-pool.ts
  docker exec "$CONTAINER" sed -i '1i\// [hotfix] sendDimensions applied' /app/src/core/store/store-pool.ts
  docker restart "$CONTAINER" >/dev/null
  wait_healthy "$CONTAINER" 90
  ok "  sendDimensions hotfix applied & restarted"
else
  ok "  sendDimensions hotfix already applied"
fi

# ── PostgreSQL 迁移补丁 ──
info "  Applying PostgreSQL migration patches..."
if [[ -x "$SCRIPT_DIR/apply-pg-patches.sh" ]]; then
  "$SCRIPT_DIR/apply-pg-patches.sh" "$CONTAINER"
  ok "  PostgreSQL patches applied"
else
  warn "  apply-pg-patches.sh not found, skipping PG patches"
fi
