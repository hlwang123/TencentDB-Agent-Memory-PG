...
import { PgSkillStore } from "./skill/pg-skill-store.js";
...
export class TdaiCore {
  ...
  private async doWireSkillModule(): Promise<void> {
    ...
    try {
      ...
      // Open the underlying storage (raw handle escape hatch — see
      // VectorStore.getRawDb() docstring). Skill tables (skills /
      // skill_vec / task_*) live in the SAME connection.
      // [pg-align] SQLite 走 getRawDb()（DatabaseSync）；PostgreSQL 走
      // PgMemoryStore.getPgPool()（共享连接池），两者角色对等。
      ...
      if (typeof rawDbCarrier.getRawDb === "function") {
        ...
        const sqliteStore = new SqliteSkillStore({
          db,
          dimensions,
          logger: this.logger,
        });
        ...
        this.logger.warn(
          `${TAG} Skill wiring skipped: vectorStore exposes neither getRawDb() (sqlite) nor getPgPool() (postgres)`,
        );
        return;
      }
