...
Integrates: Memory V2 API + MCP Knowledge Server + DashScope LLM
"""
...
async def get_tools():
    ...
    return {"tools": all_tools, "mcp": mcp_tools_names, "memory": ["memory_search", "conversation_search", "user_persona", "list_scenarios"]}
                ...
                if node_name == "agent":
                    ...
                    for tm in tool_msgs:
                        steps.append({
                            ...
                            "result": (tm.content or "")[:300],
                        })
    except Exception as e:
        final_answer = f"Agent error: {e}"
        print(f"[ERROR] {e}")

    # Save conversation to memory (per-user session)
    if final_answer:
        try:
            await client.add_conversation(f"default:{user_id}", [
...
@app.get("/api/health")
async def health():
    return {"status": "ok", "mcp": MCP_URL, "memory": MEMORY_URL, "llm": LLM_MODEL}
